...
describe('OfxToBankAccount', () => {
    ...
    describe('loadOfxFile', async () => {
        test('loadOfxFile calls ofxToBankAccount', async () => {
            ...
            const fileReader = {
                ...
                readAsText(blob: Blob, encoding?: string) {
                    if (fileReader.onload) {
                        const ev: ProgressEvent<FileReader> = new ProgressEvent(
                            'load',
                            {}
            ...
            vi.spyOn(ofxToBankAccount, 'convertOfxToBankAccount').mockImplementation(
                (ofxContent) => {
                    return {
                        name: 'account',
                        accountId: '1234',
        ...
        test('reject if convertOfxToBankAccount returns undefined', async () => {
            const file = new File([''], 'file.ofx')
            const fileReader = {
                result: '',
                readAsText(blob: Blob, encoding?: string) {
            ...
            vi.spyOn(ofxToBankAccount, 'convertOfxToBankAccount').mockImplementation(
                (ofxContent): BankAccount | undefined => {
                    return
                }
            )
    ...
    describe('convertOfxToBankAccount', async () => {
        test('convertOfxToBankAccount returns undefined if document not found', async () => {
        ...
        test('return undefined if an exception is thrown', async () => {
            vi.spyOn(ofxParser, 'parse').mockReturnValue({
                document: {
                    version: '',
                    security: '',
            ...
            vi.spyOn(ofxToBankAccount, 'createAccountFromOfxDocument').mockImplementation(() => {
        ...
        test('convertOfxToBankAccount success', async () => {
            ...
            const fctSpy = vi
                .spyOn(ofxToBankAccount, 'createAccountFromOfxDocument')
                .mockReturnValue(fctRet)
    ...
    describe('ofxToBankAccountTransaction', async () => {
        ...
        test('OfxToBankAccountTransaction throws error if fitId not found', () => {
            ...
            expect(() => ofxToBankAccount.OfxToBankAccountTransaction(ofxTransaction)).toThrowError(
                'Transaction ID not found in OFX file.'
            )
        })
        ...
        test('OfxToBankAccountTransaction throws error if datePosted not found', () => {
            ...
            expect(() => ofxToBankAccount.OfxToBankAccountTransaction(ofxTransaction)).toThrowError(
                'Transaction date not found in OFX file.'
            )
        })
        ...
        test('OfxToBankAccountTransaction throws error if amount not found', () => {
            ...
            expect(() => ofxToBankAccount.OfxToBankAccountTransaction(ofxTransaction)).toThrowError(
                'Transaction amount not found in OFX file.'
            )
        })
        ...
        test('OfxToBankAccountTransaction throws error if type not found', () => {
            ...
            expect(() => ofxToBankAccount.OfxToBankAccountTransaction(ofxTransaction)).toThrowError(
                'Transaction type not found in OFX file.'
            )
        })
    })

    describe('createAccountFromOfxDocument', async () => {
        ...
        test('createAccountFromOfxDocument throws error if accountId not found', () => {
            ...
            expect(() => ofxToBankAccount.createAccountFromOfxDocument(document)).toThrowError(
                'Account ID not found in OFX file.'
            )
        })
        ...
        test('createAccountFromOfxDocument throws error if startDate not found', () => {
            ...
            expect(() => ofxToBankAccount.createAccountFromOfxDocument(document)).toThrowError(
                'Start date not found in OFX file.'
            )
        })
        ...
        test('createAccountFromOfxDocument throws error if endDate not found', () => {
            ...
            expect(() => ofxToBankAccount.createAccountFromOfxDocument(document)).toThrowError(
                'End date not found in OFX file.'
            )
        })
    })
})
